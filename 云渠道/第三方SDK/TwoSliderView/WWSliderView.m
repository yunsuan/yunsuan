...
- (void) setLayerFrames
{
    CGFloat widthRates = [UIScreen mainScreen].bounds.size.width/360;
    track_x = 25 * widthRates;
...
//    NSString *leftStr = (NSInteger)_lowerValue < 10 ? @"0" : @"";
    ...
    leftLabel.text = [NSString stringWithFormat:@"%ld",(NSInteger)_lowerValue];
    rightLabel.text = [NSString stringWithFormat:@"%@%ld",rightStr,(NSInteger)_upperValue];
...
- (BOOL)continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if (_seclectedLowerKnob)
    {
        ...
        //左边滑块距右边滑块最小间距为1
        if (_lowerValue + 100 > _upperValue) {
            _lowerValue = _upperValue - 100;
        }
    }
    if (_seclectedUpperKnob)
    {
