...
@implementation RecommendThreeImageCell
...
- (void)initUI{
    ...
    _headImg2 = [[UIImageView alloc] init];//WithFrame:CGRectMake(12 *SIZE, 16 *SIZE, 100 *SIZE, 88 *SIZE)];
    _headImg2.contentMode = UIViewContentModeScaleAspectFill;
    _headImg2.clipsToBounds = YES;
    [self.contentView addSubview:_headImg2];
    
    _headImg3 = [[UIImageView alloc] init];//WithFrame:CGRectMake(12 *SIZE, 16 *SIZE, 100 *SIZE, 88 *SIZE)];
    _headImg3.contentMode = UIViewContentModeScaleAspectFill;
    _headImg3.clipsToBounds = YES;
    [self.contentView addSubview:_headImg3];
...
- (void)masonryUI{
    ...
    [_headImg2 mas_makeConstraints:^(MASConstraintMaker *make) {
        
        make.left.equalTo(self.contentView).offset(128 *SIZE);
        make.top.equalTo(_titleL.mas_bottom).offset(8 *SIZE);
        make.width.mas_equalTo(104 *SIZE);
        make.height.mas_equalTo(88 *SIZE);
    }];
