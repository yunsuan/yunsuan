...
@implementation SecComRoomProjectVC
...
- (void)RequestMethod{
    ...
    [BaseRequest GET:HouseProjectDetail_URL parameters:dic success:^(id resposeObject) {
        //        NSLog(@"%@",resposeObject);
        if ([resposeObject[@"code"] integerValue] == 200) {
    ...
    if (!section) {
        ...
        header.secComHeaderTagBlock = ^(NSInteger btnNum) {
            ...
            if (btnNum == 1) {
                ...
                SecComAllRoomListVC *nextVC = [[SecComAllRoomListVC alloc] initWithProjectId:_projectId city:_city];
                [self.navigationController pushViewController:nextVC animated:YES];
            }else if (btnNum == 3){
