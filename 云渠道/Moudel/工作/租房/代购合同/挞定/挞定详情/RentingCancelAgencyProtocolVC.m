//
//  RentingCancelAgencyProtocolVC.m
//  云渠道
//
...
@interface RentingCancelAgencyProtocolVC ()
{
...
@implementation RentingCancelAgencyProtocolVC
...
-(void)action_type
{
    SinglePickView *view = [[SinglePickView alloc]initWithFrame:self.view.frame WithData:[self getDetailConfigArrByConfigState:RENT_BREACH_TYPE]];
...
-(void)action_confirm
{
    ...
    [BaseRequest POST:RentSubBreach_URL parameters:dic success:^(id resposeObject) {
        NSLog(@"%@",resposeObject);
        if ([resposeObject[@"code"] integerValue] == 200) {
            
            for (UIViewController *vc in self.navigationController.viewControllers) {
                
                if ([vc isKindOfClass:[RentingRoomAgencyVC class]]) {
                    
                    if (self.rentingCancelAgencyProtocolVCBlock) {
                        
                        self.rentingCancelAgencyProtocolVCBlock();
                    }
