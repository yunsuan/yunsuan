//
//  RentAgencyEditCustomerDetailVC.m
//  云渠道
//
...
@interface RentAgencyEditCustomerDetailVC ()
{
...
@implementation RentAgencyEditCustomerDetailVC
...
- (void)ActionSaveBtn:(UIButton *)btn{
    ...
    if ([self.status isEqualToString:@"添加"]) {
        ...
        [BaseRequest POST:RentSubClientAdd_URL parameters:dic success:^(id resposeObject) {
            
            NSLog(@"%@",resposeObject);
            if ([resposeObject[@"code"] integerValue] == 200) {
                
                if (self.rentAgencyEditCustomerDetailVCBlock) {
                    
                    self.rentAgencyAddCustomerDetailVCBlock();
                }
                [self alertControllerWithNsstring:@"温馨提示" And:@"添加成功" WithDefaultBlack:^{
                    
                    for (UIViewController *vc in self.navigationController.viewControllers) {
                        
                        if ([vc isKindOfClass:[RentingAgencyDoneDetailVC class]]) {
        ...
        [BaseRequest POST:RentSubClientUpdate_URL parameters:dic success:^(id resposeObject) {
            ...
            if ([resposeObject[@"code"] integerValue] == 200) {
                ...
                if (self.rentAgencyEditCustomerDetailVCBlock) {
                    ...
                    self.rentAgencyEditCustomerDetailVCBlock(dic);
                }
