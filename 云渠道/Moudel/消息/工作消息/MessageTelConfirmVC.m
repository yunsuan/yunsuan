...
@interface MessageTelConfirmVC ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation MessageTelConfirmVC
...
- (void)RequestMethod{
    ...
    [BaseRequest GET:@"agent/message/work/telCheck" parameters:@{@"message_id":_messageId,@"client_id":_clientId} success:^(id resposeObject) {
    ...
    UIAlertAction *unuse = [UIAlertAction actionWithTitle:@"可带看" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [BaseRequest GET:ClientTelCheckValue_URL parameters:@{@"client_id":_clientId} success:^(id resposeObject) {
            ...
            if ([resposeObject[@"code"] integerValue] == 200) {
                ...
                if (self.messageTelConfirmVCBlock) {
                    
                    self.messageTelConfirmVCBlock();
                    [self.navigationController popViewControllerAnimated:YES];
                }
